...
class SelfAttention(nn.Module):
    ...
    def forward(self, query, edge_index):
        # 计算注意力权重
        src, dst = edge_index

        q = self.W_query(query)
        k = self.W_key(query)
        v = self.W_value(query)
        # edge features
        q_src = q[src]
        k_dst = k[dst]

        edge_feat = torch.cat([q_src, k_dst], dim=-1)

        scores = self.leaky_relu(edge_feat @ self.a.T).squeeze(-1)

        alpha = torch.softmax(scores, dim=0)

        out = torch.zeros_like(query)

        out.index_add_(0, src, alpha.unsqueeze(-1) * v[dst])
...
from src.utils.utils import initialize_weights
